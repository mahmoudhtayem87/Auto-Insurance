...
@Component({
	templateUrl:
		...
		'/o/InsuranceQouteCalulatorAuto/app/app.component.html'
})
export class AppComponent {
	...
	public get Quotation() {
		return (parseFloat(this.QuoteForm.get("CarValue").value)  ) * this.Rate;
	}
	...
	constructor(public http: HttpClient) {
		...
		this.QuoteForm = new FormGroup
			({
				LicenceHeldFor: new FormControl('', Validators.required),
				YearManufactured: new FormControl('', Validators.required),
				CarMake: new FormControl('', Validators.required),
				CarModel: new FormControl('', Validators.required),
				CityOfRegistration: new FormControl('', Validators.required),

				CurrentlyWith: new FormControl('', Validators.required),
				CarValue: new FormControl('', Validators.required),
	...
	public ngAfterViewInit() {
		this.loadCountries();
		//console.log(this.configurationJSON);
		$("#QuotationSlider").carousel('pause');
	}
	...
	public next() {
		...
		$("#QuotationSlider").carousel('next');
		console.log(this.QuoteForm.get("CarValue").value);
		console.log(this.Quotation);
	}
	public back() {
		this.validateAllFormFields(this.QuoteForm);
		$("#QuotationSlider").carousel('prev');
	}
	public onSubmit() {
		this.validateAllFormFields(this.QuoteForm);
	...
	public get VehiclesMakes() {
		return this._VehiclesMakes.Makes;
	}
	public VehicleModels: any = [];
	public OnMakeSelected(make: string) {
		this.VehicleModels = this._VehiclesMakes.getVehiclesByMake(make);
	}
	...
	public get AppLabels() {
		try {
			...
			//console.log(labels);
			if (labels == null || labels == "" || labels == "-1") {
			...
			// invalid translation object in the application system scoped properties
			return this.Labels.Data["en_US"];
		}
